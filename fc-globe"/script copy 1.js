...
if (window.location.pathname == '/home/') {
	...
	fetch('https://shop.theoneinamillionglobe.com/wp-json/image-api/image-item/').then((res) => res.text()).then((data) => {
		ApiImage = JSON.parse(data);
	});

	let Arr = [];
	let allCat;
	fetch('https://shop.theoneinamillionglobe.com/wp-json/categories-api/categories-item/')
		.then((res) => res.text())
...
`;
	...
	function searchData() {
		...
		if (Cat) {
			isSearching = true;
			var url = new URL('https://shop.theoneinamillionglobe.com/wp-json/search-cluster-api/search-item'),
				params = { ...Cat };
  ...
  `;
	...
	var myGlobe = Globe()(document.getElementById('globeViz'))
		.globeImageUrl('https://shop.theoneinamillionglobe.com/wp-content/uploads/2020/06/globeimg.gif')
		.backgroundImageUrl('https://shop.theoneinamillionglobe.com/wp-content/uploads/2020/06/blackbg.png')
	...
	var getData = (CATEGORY) =>
		Promise.all([
			fetch('https://shop.theoneinamillionglobe.com/wp-json/map-api/distance-item').then((res) => res.text())
		]).then(([ data ]) => {
